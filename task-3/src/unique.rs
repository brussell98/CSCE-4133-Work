...
fn answer_queries(seq: Vec<usize>, queries: Vec<[usize; 3]>) {
	...
	// Binary Index Tree representing the number of distinct elements last occurring at i
	...
	for i in 0..seq.len() {
		...
		while q_count < queries.len() && queries[q_count][1] == i {
			...
			// Sum the number of distinct elements so far, and subtract the number of
			// Distinct elements that haven't occurred after left.
